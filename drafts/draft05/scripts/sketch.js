...
function setup() {
  ...
  blend = floor(random(4));
  scaleBool1 = random(1) > 0.7?true:false;
  scaleBool2 = random(1) > 0.7?true:false;

  if(random(1) > 0.5) background(bg);
  noStroke();
  buffer0.noStroke();
  buffer1.noStroke();
...
function draw() {
  ...
  // buffer1.translate(0,s);
    // buffer1.scale(1.0999999999999,1.00001);
  if (scaleBool1) {
  buffer1.translate(0,-0.9);
  }
  if (scaleBool2) {
    buffer1.translate(0,sin(frameCount*0.01)*0.1);
  }
  // buffer1.rotateZ(sin(frameCount * 0.1)*1);
    buffer1.scale(0.9999999999,sin(frameCount * 0.1)*0.000001+1.00001);

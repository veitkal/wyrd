...
let numWarps, numRepeats, numVert, orgX, orgY, offX, offY, sz, threadSize, wWidth, wHeight, warpLength, shapeCount, treadlingCount;
...
function calcTreadling(_arr) {
  ...
  flatArr.forEach((el, index) => {
    let tempPos = createVector(orgX + shapeBox.w + (index * threadSize), el.newPos.y);
    let tempVert = new Vert(tempPos);
    treadlingArr.push(tempVert);
    treadlingArr[index].newPos = tempPos;
    treadlingX.push(orgX + shapeBox.w + (index * threadSize));
  });
  } else {
    //sorting the repeat, copying n times and flatten into a single array
    for (let i = 0; i < numRepeats; i++) {
...
function treadlingUpdateIdx(_idx) {
  ...
  if (treadlingCount < treadlingArr.length - 1) {
    ...
    treadlingArr[_idx].rePos(createVector(treadlingX[_idx], tempY));
    shapeCount++;

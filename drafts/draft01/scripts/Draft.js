...
class Draft {
  ...
  updateThreading() {
    //updating threading as waveform. 
    if (frameCount%this.updateRate === 0) {
      let noiseVal = map(noise(millis()*sin(frameCount*this.osc1_noise)), 0, 1, -1, 1);
      let freq = sin(frameCount*this.osc1_freq2);
      let freqMod = sin(millis() * 0.001);
      let wave = ((sin((millis())  * this.osc1_freq1)+freq + noiseVal));
      let tempVal = round(constrain(map(wave, -1, 1, 0, this.numShafts-1), 0, this.numShafts-1));
      // this.threading.push(tempVal);
      // this.threading.shift();
  ...
  updateTreadling() {
    ...
    if (frameCount%this.updateRate === 0) {
      ...
      let noiseVal = map(noise(millis()*sin(frameCount*this.osc2_noise)), 0, 1, -1, 1);
      let freq = sin(frameCount*this.osc2_freq2);
      let freqMod = sin(millis() * 0.001);
      let wave = ((sin((millis())  + freqMod * this.osc2_freq1)+freq + noiseVal));
      let tempVal = round(constrain(map(wave, -1, 1, 0, this.numShafts-1), 0, this.numShafts - 1));
      // this.threading.push(tempVal);
      // this.threading.shift();
  ...
  threadingBounds(_x, _y) {
    //check and return if xy is within bounds
    return (_x > this.originOff && _x < this.width && _y > this.originOff && _y < this.originOff + this.cellWidth * this.numShafts);
  }
  ...
  tieUpBounds(_x, _y) {
    ...
    rect(tieUpX,tieUpY, this.boxWidth, this.boxWidth);
    return (_x > tieUpX && _x < tieUpX + this.boxWidth && _y > tieUpY && _y < tieUpY + this.cellWidth * this.numShafts);
  }

  treadlingBounds(_x, _y) {
    //check and return if xy is within bounds
    let treadlingX = this.originOff + this.width + this.boxPad;
    let treadlingY = this.originOff + this.boxWidth + this.boxPad;

    rect(treadlingX, treadlingY, this.boxWidth, this.height);
    return (_x > treadlingX && _x < treadlingX + this.boxWidth && _y > treadlingY && _y < treadlingY + this.cellWidth * this.numWeft);
  }
  drawDownBounds(_x, _y) {
    //check and return if xy is within bounds
    let drawDownX = this.originOff;
    let drawDownY = this.originOff + this.boxWidth+this.boxPad;
    ...
    ////draw box
    rect(drawDownX, drawDownY, this.width, this.height);
    return (_x > drawDownX && _x < drawDownX + this.width && _y > drawDownY && _y < drawDownY + this.height);
  }

  clickCheck(_x, _y, _dragged, _tieUpOnly) {
  ...
  randomizeOscVals() {
    let scaleBool = false;
    if (scaleBool) {
      this.osc1_freq1 = random(1);
      this.osc1_freq2 = random(1);
      this.osc1_noise = random(1);
      this.osc2_freq1 = random(1);
      this.osc2_freq2 = random(1);
      this.osc2_noise = random(1);
    } else {
      this.osc1_freq1 = random(0.001);
      this.osc1_freq2 = random(0.001);
      this.osc1_noise = random(0.001);
      this.osc2_freq1 = random(0.001);
      this.osc2_freq2 = random(0.001);
      this.osc2_noise = random(0.001);
    }
    scaleBool = !scaleBool;
  }
}
